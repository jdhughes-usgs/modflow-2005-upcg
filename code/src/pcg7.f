      MODULE PCGMODULE
        ...
        INTEGER,SAVE,POINTER  ::IPCGO,IPCGI
        REAL   ,SAVE,POINTER  ::DAMPPCGT
        DOUBLE PRECISION, SAVE, POINTER :: PCGTOTT, PCGFMAT
      ...
      TYPE PCGTYPE
        ...
        INTEGER,POINTER  ::IPCGO,IPCGI
        DOUBLE PRECISION, POINTER :: PCGTOTT, PCGFMAT
...
C     ------------------------------------------------------------------
      ...
      ALLOCATE(IPCGO,IPCGI)
      ALLOCATE(PCGTOTT,PCGFMAT)
      IPCGO   = 0
      IPCGI   = 0
      PCGTOTT = 0.0D0
      PCGFMAT = 0.0D0
...
C     ------------------------------------------------------------------
     ...
     &     DAMPSS, DAMPTR,HDRY
      INTEGER IPCGO,IPCGI
      DOUBLE PRECISION PCGTOTT, PCGFMAT
...
C     ------------------------------------------------------------------
C
C-------START PCG TOTAL TIMER
      CALL SPCGTIMER(0,tt1,PCGTOTT)
C
C-------START FORMULATE TIMER
      CALL SPCGTIMER(0,ttf,PCGFMAT)
C
      IF(ISS.EQ.0) THEN
        DAMP=DAMPTR
      ELSE
...
C
      CALL PCG7PNT(IGRID)
        ...
        DEALLOCATE(IPCGO,IPCGI)
        DEALLOCATE(PCGTOTT,PCGFMAT)
        DEALLOCATE(VPCG)
        DEALLOCATE(SS)
        DEALLOCATE(P)
        DEALLOCATE(HPCG)
...
C
      ...
      IPCGO=>PCGDAT(IGRID)%IPCGO
      IPCGI=>PCGDAT(IGRID)%IPCGI
      PCGTOTT=>PCGDAT(IGRID)%PCGTOTT
      PCGFMAT=>PCGDAT(IGRID)%PCGFMAT
...
C
      ...
      PCGDAT(IGRID)%IPCGO=>IPCGO
      PCGDAT(IGRID)%IPCGI=>IPCGI
      PCGDAT(IGRID)%PCGTOTT=>PCGTOTT
      PCGDAT(IGRID)%PCGFMAT=>PCGFMAT
...
C
      ...
      SUBROUTINE PCG7OT(IGRID)
C     ******************************************************************
C     OUTPUT PCG TIMER RESULTS - FLOATING POINT OPERATIONS
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C     + + + LOCAL DEFINITIONS + + +
      DOUBLEPRECISION :: tpcg
C     + + + FUNCTIONS + + +
C     + + + FORMATS + + +
2000  FORMAT(//,1X,'SUMMARY OF PCG EXECUTION TIME',
     &        /,1X,'TOTAL NUMBER OF OUTER PCG ITERATIONS: ',I10,
     &        /,1X,'TOTAL NUMBER OF INNER PCG ITERATIONS: ',I10,
     &       //,1X,'TIMER ITEM',32X,'TIME (SEC.)',4X,' PERCENTAGE',
     &        /,1X,69('-'),
     &        /,1X,'TOTAL PCG EXECUTION TIME:              ',G16.7,
     &          1X,F10.3,
     &        /,1X,'TOTAL PCG A MATRIX FORMULATE TIME:     ',G16.7,
     &          1X,F10.3,
     &        /,1X,69('-'))
C     + + + CODE + + +
C
...
C
        tpcg = 1.0D0
        IF ( PCGTOTT.GT.0.0D0 ) tpcg = tpcg / PCGTOTT
        WRITE (IOUT,2000) IPCGO, IPCGI, 
     &                    PCGTOTT,  100.0D0 * PCGTOTT * tpcg,
     &                    PCGFMAT,  100.0D0 * PCGFMAT * tpcg
C
C-------RETURN
      RETURN
C
      END SUBROUTINE PCG7OT
C
C-------TIMER FOR PCG CALCULATIONS
      SUBROUTINE SPCGTIMER(It,T1,Ts)
        IMPLICIT NONE
C     + + + DUMMY ARGUMENTS + + +
