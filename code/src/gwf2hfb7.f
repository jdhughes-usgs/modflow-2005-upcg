...
C3------READ AND PRINT NPHFB, MXFB, NHFBNP
  ...
  530 FORMAT(1X,I6,' MAXIMUM HORIZONTAL FLOW BARRIERS NOT DEFINED BY',
     &       ' PARAMETERS')
C
C4------LOOK FOR NOPRINT OPTION AND TRANSIENT OPTION.
      IPRHFB = 1
      ITRHFB = 0
00010 CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,INHFB)
    ...
    3   FORMAT(1X,
     &'LISTS OF HORIZONTAL FLOW BARRIER CELLS WILL NOT BE PRINTED')
        IPRHFB = 0
        GOTO 10
      ELSE IF(LINE(ISTART:ISTOP).EQ.'TRANSIENT') THEN
        WRITE(IOUT,4)
    4   FORMAT(1X,
     &'TRANSIENT HORIZONTAL FLOW BARRIER CELLS WILL BE SPECIFIED')
        ITRHFB = 1
        GOTO 10
      END IF
C
...
C6------IS STILL NEGATIVE, IT IS ASSUMED THAT HFB WILL NOT WORK.
  ...
  550   FORMAT(/,
     ...
     &' -- STOP EXECUTION (GWF2HFB7AR)')
      ...
      ENDIF
C
C7------READ AND WRITE MAXIMUM NUMBER OF PARAMETERS
      WRITE(IOUT,600) NPHFB
  600 FORMAT(//,1X,I5,' Maximum HFB parameters')
C7------READ PARAMETER DEFINITIONS (ITEMS 2 AND 3)
   ...
   20   CONTINUE
      ENDIF
C
C8------SAVE POINTERS TO GRID AND RETURN.
      CALL SGWF2HFB7PSV(IGRID)
      RETURN
      END SUBROUTINE GWF2HFB7AR

      SUBROUTINE GWF2HFB7RP(INHFB,KKPER,IGRID)
C     ******************************************************************
C     READ DATA FOR HORIZONTAL FLOW BARRIER PACKAGE
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C
      ...
      IHFBNP = NHFBNP
C
C------DETERMINE IF DATA SHOULD BE READ FOR THIS STRESS PERIOD
C     
      IF ( ITRHFB.LT.1 .AND. KKPER.GT.1 ) RETURN      
C
C-------READ TEMPORARY
      IF ( ITRHFB.GT.0 ) THEN
        CALL URDCOM(INHFB,IOUT,LINE)
        LLOC = 1
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,2,ITMP,DUM,IOUT,INHFB)
        IF ( ITMP.LT.0 ) THEN
          IF ( KKPER.GT.1 ) THEN
            WRITE(IOUT,200)
            RETURN
          ELSE
            WRITE(IOUT,210)
            CALL USTOP('ERROR: CANNOT REUSE HFB ON FIRST STRESS PERIOD')
          END IF
        END IF
C         RESET HORIZONTAL BRANCH CONDUCTANCES FOR CONSTANT T LAYERS
        IF ( KKPER.GT.1 ) THEN
          CALL SGWF2HFB7RC()
        END IF      
C         SET HFB DIMENSIONS FOR HORIZONTAL FLOW BARRIERS NOT DEFINED BY 
C         PARAMETERS FOR STRESS PERIOD
        IHFBNP = ITMP
        WRITE(IOUT,210) IHFBNP
      END IF
  200 FORMAT(1X,'REUSING HORIZONTAL FLOW BARRIERS FROM FROM ',
     2          'LAST STRESS PERIOD')
  210 FORMAT(1X,I6,' HORIZONTAL FLOW BARRIERS NOT DEFINED BY',
     &       ' PARAMETERS')

C
C8------READ BARRIERS NOT DEFINED BY PARAMETERS (ITEM 4)
      NHFB = 0
      WRITE(IOUT,610) IHFBNP
  610 FORMAT(/,1X,I6,' BARRIERS NOT DEFINED BY PARAMETERS')
      IF(IHFBNP.GT.0) THEN
        CALL SGWF2HFB7RL(IHFBNP,HFB,1,MXHFB,INHFB,IOUT,
     1      'BARRIER  LAYER  IROW1  ICOL1  IROW2  ICOL2    HYDCHR',
     2             NCOL,NROW,NLAY,IPRHFB)
        NHFB = NHFB + IHFBNP
        CALL SGWF2HFB7CK(1,IHFBNP)
      ENDIF
C
...
C16-----RETURN
      RETURN
      END
      SUBROUTINE SGWF2HFB7RC()
C     ******************************************************************
C     RESET HORIZONTAL CONDUCTANCES (CR AND CC) FOR CONFINED LAYERS TO
C     REMOVE HORIZONTAL FLOW BARRIERS.  THIS SUBROUTINE IS CALLED WHEN
C     NEW HORIZONTAL FLOW BARRIER DATA IS READ AT THE BEGINNING OF A
C     STRESS PERIOD FOR STRESS PERIODS GREATER THAN 1.
C     ******************************************************************
C
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C-------INITIALIZE ERROR FLAG TO ZERO.
      IERFLG=0
C
C-----DO FOR EACH BARRIER IN RANGE.
      DO 10 II = 1,NHFB
        K = HFB(1,II)
C
C-------FIND ROW AND COLUMN NUMBERS OF THE TWO CELLS ON BOTH SIDES
C-------OF THE BARRIER.
        I1 = HFB(2,II)
        J1 = HFB(3,II)
        I2 = HFB(4,II)
        J2 = HFB(5,II)
C
C-------IF I1=I2, BARRIER IS BETWEEN TWO CELLS ON THE SAME ROW.
        IF (I1.EQ.I2) THEN
C
C-------IF J2-J1=1, THE TWO CELLS ARE NEXT TO ONE ANOTHER (DATA OK).
          IF ((J2-J1).EQ.1) THEN
C
C-------BARRIER CELLS ARE ADJACENT.
C-------IF LAYER IS CONFINED AND BOTH CELLS ARE ACTIVE, SAVE
C-------ORIGINAL CR FOR COMPUTING SENSITIVITIES AND MODIFY CR
            IF (LAYHDT(K).EQ.0) THEN
C
C-------IF CR(J1,I1,K) NOT 0, BOTH CELLS ARE ACTIVE.
              IF (CR(J1,I1,K).NE.0.) THEN
                CR(J1,I1,K) = HFB(7,II)
              ENDIF
          ...
          ENDIF
C
C-------IF J1=J2, BARRIER IS BETWEEN TWO CELLS ON THE SAME COLUMN.
        ELSEIF (J1.EQ.J2) THEN
C
C-------IF I2-I1=1, THE TWO CELLS ARE NEXT TO ONE ANOTHER (DATA OK).
          IF ((I2-I1).EQ.1) THEN
C
C-------BARRIER CELLS ARE ADJACENT.
C-------IF LAYER IS CONFINED AND BOTH CELLS ARE ACTIVE, SAVE
C-------ORIGINAL CC FOR COMPUTING SENSITIVITIES AND MODIFY CC
            IF (LAYHDT(K).EQ.0) THEN
C
C-------IF CC(J1,I1,K) NOT 0, BOTH CELLS ARE ACTIVE.
              IF (CC(J1,I1,K).NE.0.) THEN
                CC(J1,I1,K) = HFB(7,II)
              ENDIF
...
C
        ...
        DEALLOCATE(GWFHFBDAT(IGRID)%ITRHFB)
C
      RETURN
      END
